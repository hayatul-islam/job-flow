...
import { SEARCH_JOB_TYPES, SEARCH_QUICK_TAGS } from "@/lib/data";
...
export default function SearchBarLight() {
  ...
  const selectedOption =
    SEARCH_JOB_TYPES.find((t) => t.value === jobType) ?? SEARCH_JOB_TYPES[0];
  ...
  const handleQuickTag = (tag: (typeof SEARCH_QUICK_TAGS)[number]) => {
    if (tag.q) setQ((prev) => (prev === tag.q ? "" : tag.q!));
    if (tag.jobType)
      setJobType((prev) => (prev === tag.jobType ? "" : tag.jobType!));
  };

  const isTagActive = (tag: (typeof SEARCH_QUICK_TAGS)[number]) =>
  ...
  return (
    <motion.div
      className="w-full max-w-3xl mx-auto lg:mx-0 lg:mb-16"
      variants={stagger}
      initial="hidden"
      animate="show"
    >
      <motion.div variants={fadeUp}>
        <Card className="p-0 flex flex-col sm:flex-row">
          ...
          <div
            ref={triggerRef}
            className="relative flex items-center gap-2 px-4 py-1 cursor-pointer select-none sm:min-w-[160px]"
      ...
      <motion.div
        className="flex flex-wrap items-center gap-2 mt-3 px-1 justify-center lg:justify-start"
        variants={fadeUp}
      >
        ...
        {SEARCH_QUICK_TAGS.map((tag) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] sm:text-xs font-medium transition-all hover:scale-[1.03] text-primary/60 hover:text-primary/80 border ${active ? "bg-primary/5 border-primary/40 text-primary/80" : "bg-primary/5 border-primary/5"}`}
            >
