...
export default function JobCardV2({ job }: JobCardProps) {
  ...
  return (
    <Link
      href={`/jobs/${job.id}`}
      className="group block bg-white border border-gray-200 rounded-xl overflow-hidden hover:border-gray-400 transition-all duration-150 cursor-pointer"
    >
      <div className="flex items-center justify-between gap-3 px-4 py-2.5 bg-gray-50 border-b border-gray-200">
        <div className="flex items-center gap-2.5 min-w-0">
          <div className="w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center text-xs font-semibold text-gray-700 shrink-0">
            {initial}
          </div>
          <div className="min-w-0">
            ...
            <p className="flex items-center gap-1 text-[11px] text-gray-500 leading-tight">
